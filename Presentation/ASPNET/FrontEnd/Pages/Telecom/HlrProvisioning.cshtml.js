const HLR_LOG_VIEWER_ROLES = new Set(['TelecomAdmin', 'TelecomBackOffice', 'TelecomManagement']);
...
const HlrProvisioningApp = {
    setup() {
        const roles = StorageManager.getUserRoles() || [];
        const canAccess = roles.some((r) => HLR_LOG_VIEWER_ROLES.has(r));
        ...
        const loadPage = async () => {
            ...
            try {
                ...
                const res = await AxiosManager.get('/Telecom/GetHlrProvisioningLogList', { params });
        ...
        const pageLabel = Vue.computed(() => {
            const tpl =
                (typeof window.TelecomI18n !== 'undefined' && window.TelecomI18n.t('hlrProvisioning.pageOf')) ||
                'Page {page} of {total}';
...
if (typeof Vue !== 'undefined') {
    Vue.createApp(HlrProvisioningApp).mount('#app');
} else {
